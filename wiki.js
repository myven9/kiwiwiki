...
// 이미지 버튼을 누른 시점의 대상 textarea를 "직접" 저장하면, 파일 선택창이 열려있는 동안(특히 모바일에서
// 사진을 고르는 몇~몇십 초 사이) 다른 이유로 renderPage()가 한 번이라도 실행될 경우 그 textarea가
// 화면에서 사라진(detached) 옛 DOM을 가리키게 되어, 업로드 자체는 성공해도 화면에는 반영되지 않는
// 문제가 생긴다. 그래서 요소를 직접 들고 있지 않고, "최신 DOM에서 다시 찾는 함수(locator)"를 저장해서
// 실제로 삽입하는 시점에 다시 조회하도록 한다.
let pendingImageLocator = null; // () => 대상 textarea 요소를 반환하는 함수
...
function setupImageUpload() {
  ...
  input.addEventListener("change", async () => {
    ...
    const locator = pendingImageLocator;
    const cursorPos = pendingImageCursorPos;
    const button = pendingImageButton;
    pendingImageLocator = null;
    pendingImageCursorPos = null;
    pendingImageButton = null;
    if (!file || !locator) return;

    // 파일 선택창이 열려있던 동안 화면이 다시 그려졌을 수 있으므로, 여기서 최신 DOM 기준으로 다시 찾는다.
    const textarea = locator();
    if (!textarea) {
      alert("이미지를 삽입할 위치를 찾지 못했습니다. 편집 중이던 문단/항목을 다시 열고 시도해주세요.");
      return;
    }
...
// 이미지 버튼 클릭 시점의 커서 위치를 미리 저장해둔다 (파일 선택창이 열리는 동안 focus가 빠지기 때문)
// locator: () => 삽입 대상 textarea를 "최신 DOM에서" 반환하는 함수 (요소를 직접 저장하지 않는 이유는
// 파일 선택 중 renderPage()가 실행되면 직접 저장된 요소가 화면에서 사라진 옛 DOM이 되어버리기 때문)
// triggerBtn: 업로드 중 라벨을 바꿔줄 버튼
function handleImageButtonClick(locator, triggerBtn) {
  if (!locator || !imageUploadInput) return;
  const textarea = locator();
  if (!textarea) return;
  pendingImageLocator = locator;
...
function attachEditingHandlers() {
  root.addEventListener("click", async (e) => {
    ...
    if (infoboxImageBtn) {
      ...
      const rowIndex = container ? Array.from(container.children).indexOf(rowEl) : -1;
      // 요소를 직접 넘기지 않고, "같은 순번의 행"을 최신 DOM에서 다시 찾는 locator를 넘긴다
      handleImageButtonClick(() => {
        const latestContainer = root.querySelector(".infobox-rows-editor");
        const latestRow = latestContainer?.children?.[rowIndex];
        return latestRow?.querySelector(".infobox-value-input") || null;
      }, infoboxImageBtn);
      return;
    }
    ...
    if (toolBtn) {
      ...
      if (tool === "footnote") {
        ...
        const sectionId = editorEl?.dataset.sectionId;
        // 요소를 직접 넘기지 않고, 같은 문단 id의 textarea를 최신 DOM에서 다시 찾는 locator를 넘긴다
        handleImageButtonClick(
          () => root.querySelector(`#sec-${sectionId} .editor-textarea`),
          toolBtn
        );
      } else {
        const textarea = editorEl?.querySelector(".editor-textarea");
